...
if (!isset($data->id) || !isset($data->key) || !isset($data->value)) {
...
$sql_select = "SELECT * FROM usermeta WHERE user_id = ? AND meta_key = ?";
...
$stmt_select->bind_param("is", $data->id, $data->key);
$stmt_select->execute();
$result = $stmt_select->get_result();

// Перевірка, чи існує запис з таким id і meta_key
if ($result->num_rows > 0) {
    // Оновлення запису
    $sql_update = "UPDATE usermeta SET meta_value = ? WHERE user_id = ? AND meta_key = ?";
    ...
    $stmt_update->bind_param("sis", $data->value, $data->id, $data->key);
    $stmt_update->execute();
    ...
    $sql_insert = "INSERT INTO usermeta (user_id, meta_key, meta_value) VALUES (?, ?, ?)";
    ...
    $stmt_insert->bind_param("iss", $data->id, $data->key, $data->value);
    $stmt_insert->execute();

...
const FieldsBlockGroup = ({options, case_id, title="", getCaseInfo}) => {
                ...
                {options.map((field)=>{
                    ...
                    return(<>
                        ...
                        {field.type === "boolean" && <div>
                            <b>{field.name}</b>
                            <Switch defaultChecked={field.value == 1} onChange={(e) => {
                                if (e.target.checked) {
...
const FieldsBlock = ({works, contacts, another, case_id, getCaseInfo}) => {
    
    return(<div className="FieldsBlock">
        {works.length > 0 && <FieldsBlockGroup getCaseInfo={getCaseInfo} title="Робочі дані" options={works} case_id={case_id}/>}
        {contacts.length > 0 && <FieldsBlockGroup getCaseInfo={getCaseInfo} title="Контактні дані" options={contacts} case_id={case_id}/>}
        {another.length > 0 && <FieldsBlockGroup getCaseInfo={getCaseInfo} title="Дані поза групою" options={another} case_id={case_id}/>}
    </div>)
}
